...
async def fetch(session: aiohttp.ClientSession, currency: str, deviation_limit: int) -> dict:
    """
    ...
    :return:
    """
    result = dict(timestamp=None, level='INFO', trading_pair=currency, deviation=None, data={})
    ...
    try:
        ...
        deviation = True if sdev > deviation_limit else False
...
async def fetch_all(session: aiohttp.ClientSession, symbols: list, deviation: int) -> tuple:
    """
    ...
    for symbol in symbols:
        task = asyncio.create_task(fetch(session, symbol, deviation))
        tasks.append(task)
    results = await asyncio.gather(*tasks)
    return results


async def main(currency: str, deviation: int) -> None:
    """
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            async with aiohttp.ClientSession() as session:
                if currency not in symbols or currency.upper() == "ALL":  # validates currency
                    result = await fetch_all(session, symbols, deviation)
                else:
                    result = await fetch(session, currency, deviation)
